...
MAIN_URL = f'{BASE_URL}/past-years/jee/jee-main/physics'
...
def scrape_physics():
    ...
    # Find topic links
    topic_links = soup.find_all('a', href=lambda href: href and href.startswith('/past-years/jee/jee-main/physics/'))
    ...
    for topic_name, topic_url in topics.items():
        ...
        for q_link in q_links:
            ...
            if re.match(r'^\d+\s+', text):
                ...
                # Extract the exam year from the end if present
                exam_info = ""
                exam_match = re.search(r'(JEE Main.*|AIEEE.*)$', clean_text)
                if exam_match:
    ...
    output_file = os.path.join(os.path.dirname(__file__), 'examside_physics_main.json')
