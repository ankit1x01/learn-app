# Audio Lecture Game Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a 4-phase audio comprehension game (Intro → Active Listening with drag-to-fill → MCQ Quiz → Result) as a 7th tile in the existing GamesScreen.

**Architecture:** Pre-authored config in `dsa-dummy.ts` → `AudioLectureGame.tsx` with 3 inline sub-components (`IntroPhase`, `ListeningPhase`, `QuizPhase`) → reuses existing `GameWinScreen`. Audio via `window.speechSynthesis`. Drag via pointer events + `getBoundingClientRect()`.

**Tech Stack:** React 19, TypeScript, Tailwind CSS 4, Motion (motion/react), @capacitor/haptics, Web Speech API (built-in browser)
...
|---|---|---|
| Modify | `src/games/types.ts` | Add `AudioLectureConfig` interface + union |
| Modify | `src/games/data/dsa-dummy.ts` | Add `dsaAudioLecture` export |
| **Create** | `src/games/components/AudioLectureGame.tsx` | All game phases |
| Modify | `src/games/GameRunner.tsx` | Route `audio-lecture` type |
| Modify | `src/games/GamesScreen.tsx` | Add 7th tile |

---

## Task 1: Add AudioLectureConfig to types.ts

**Files:**
- Modify: `src/games/types.ts`

- [ ] **Step 1: Add the interface and update the union**

Open `src/games/types.ts`. After the `RetentionConfig` interface (around line 60), add:
...
Then update the `GameConfig` union at the bottom of the file from:
```typescript
...
Expected: no errors related to `AudioLectureConfig`.

- [ ] **Step 3: Commit**

```bash
cd "C:/Users/Ankit/Desktop/learn app" && git add src/games/types.ts && git commit -m "feat: add AudioLectureConfig type"
```
...
- [ ] **Step 1: Add the import for AudioLectureConfig**

At the top of `src/games/data/dsa-dummy.ts`, update the import line from:
```typescript
import { ThisOrThatConfig, ChronoConfig, LinksConfig, KnockoutConfig, BalloonTapConfig, RetentionConfig } from '../types'
```
to:
```typescript
import { ThisOrThatConfig, ChronoConfig, LinksConfig, KnockoutConfig, BalloonTapConfig, RetentionConfig, AudioLectureConfig } from '../types'
```

- [ ] **Step 2: Add the export at the top of the file (after the existing imports)**
...
```bash
cd "C:/Users/Ankit/Desktop/learn app" && git add src/games/data/dsa-dummy.ts && git commit -m "feat: add dsaAudioLecture dummy data"
```

---

## Task 3: Build AudioLectureGame.tsx

**Files:**
- Create: `src/games/components/AudioLectureGame.tsx`

- [ ] **Step 1: Create the file with all four sections**

Create `src/games/components/AudioLectureGame.tsx` with the following complete content:
...
function ListeningPhase({ config, chips: initialChips, onComplete }: ListeningPhaseProps) {
  ...
  // Split displayPassage on ___ → interleave blank slots
...
Expected: no errors in `AudioLectureGame.tsx`.

- [ ] **Step 3: Commit**

```bash
cd "C:/Users/Ankit/Desktop/learn app" && git add src/games/components/AudioLectureGame.tsx && git commit -m "feat: build AudioLectureGame component"
```

---

## Task 4: Wire GameRunner
...
Open `src/games/GameRunner.tsx`. Add the import:

```typescript
import { AudioLectureGame } from './components/AudioLectureGame'
```

Then add the case inside the `switch`:
...
```bash
cd "C:/Users/Ankit/Desktop/learn app" && git add src/games/GameRunner.tsx && git commit -m "feat: wire audio-lecture in GameRunner"
```

---

## Task 5: Wire GamesScreen

**Files:**
- Modify: `src/games/GamesScreen.tsx`

- [ ] **Step 1: Update imports, Tab type, GAMES array, and CONFIGS**

Open `src/games/GamesScreen.tsx`.

**1a. Update the lucide-react import** — add `Headphones`:
```typescript
import { Shuffle, Clock, Link2, Sword, ChevronLeft, Hand, Brain, Headphones } from 'lucide-react'
```

**1b. Update the `Tab` type** — add `'audio-lecture'`:
```typescript
type Tab = 'this-or-that' | 'chrono' | 'links' | 'knockout' | 'balloon-tap' | 'retention' | 'audio-lecture'
```

**1c. Add to the `GAMES` array** — append after the retention entry:
```typescript
  {
    id: 'audio-lecture',
    label: 'Lecture',
...
**1d. Update the import in `dsa-dummy.ts`** reference in GamesScreen — add `dsaAudioLecture`:
```typescript
...
```bash
cd "C:/Users/Ankit/Desktop/learn app" && git add src/games/GamesScreen.tsx && git commit -m "feat: add Lecture tile to GamesScreen"
```

---

## Task 6: Production build verify
...
Expected: `✓ built in Xs` with zero TypeScript errors and zero Vite warnings about missing exports.

- [ ] **Step 2: Commit if any minor fixes were needed**

If Step 1 required any fixes, commit them:
```bash
cd "C:/Users/Ankit/Desktop/learn app" && git add -p && git commit -m "fix: audio-lecture build errors"
```

- [ ] **Step 3: Sync to Android**

```bash
cd "C:/Users/Ankit/Desktop/learn app" && npx cap sync android
```

Expected: `✔ copy android` and `✔ update android` with no errors.
