  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 px-6 py-10 gap-6">
      ...
      <div className="w-16 h-16 rounded-full bg-[#F0FDF4] border border-[#BBF7D0] flex items-center justify-center">
        <span className="material-symbols-rounded text-[#15803D]" style={{ fontSize: 28 }}>workspace_premium</span>
      </div>

      <div className="text-center">
        <p className="text-xs font-semibold text-[#15803D] uppercase tracking-wide mb-1">That's right!</p>
        <p className="text-xl font-bold text-[var(--color-on-surface)]" style={{ fontFamily: 'Plus Jakarta Sans, system-ui' }}>
          You got it
        </p>
      </div>

      <div className="w-full bg-white border border-[var(--color-border)] rounded-2xl p-5 flex justify-around">
...
function Stat({ label, value, unit }: { label: string; value: string; unit?: string }) {
  return (
    <div className="text-center">
      <div className="text-2xl font-bold text-[var(--color-on-surface)]" style={{ fontFamily: 'Inter, system-ui' }}>
        {value}
        {unit && <span className="text-sm font-normal text-[var(--color-border)] ml-0.5">{unit}</span>}
      </div>
      <div className="text-xs font-medium text-[var(--color-on-surface-variant)] mt-0.5" style={{ fontFamily: 'Plus Jakarta Sans, system-ui' }}>
        {label}
      </div>
    </div>
  )
