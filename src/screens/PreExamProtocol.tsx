...
import { Moon, Wind, Flame, FileText, CheckCircle2, Circle, ArrowRight, Brain } from 'lucide-react';

type Screen = 'dashboard' | 'session' | 'encoding' | 'map' | 'recall' | 'complete'
            | 'elite' | 'stress' | 'ghana' | 'distractor' | 'errors' | 'mock' | 'preexam';
...
const DAILY_SCHEDULE = [
  { id: 'pranayama', icon: Wind,     title: 'Pranayama',    subtitle: '5 min · Box breathing', time: '5:55 AM', color: 'text-[#0E7490]',  bg: 'bg-[#ECFEFF]', screen: null as Screen | null },
  { id: 'recall',   icon: Brain,     title: 'Morning Recall', subtitle: '10 min · No notes',  time: '6:00 AM', color: 'text-primary',     bg: 'bg-primary/10',   screen: 'recall' as Screen },
  { id: 'ghana',    icon: Flame,     title: 'Ghana Patha',  subtitle: '30 min · Stage 4 drill', time: '6:15 AM', color: 'text-[#F59E0B]', bg: 'bg-[#F59E0B]/10', screen: 'ghana' as Screen },
  { id: 'mock',     icon: FileText,  title: 'Mock Interview',  subtitle: '45 min · FAANG Loop', time: '10:00 AM', color: 'text-[#15803D]',   bg: 'bg-[#F0FDF4]',   screen: 'mock' as Screen },
  { id: 'errors',   icon: Flame,     title: 'Error Review', subtitle: '1 hr · Fix clusters',   time: '2:00 PM', color: 'text-[#7C3AED]', bg: 'bg-[#F5F3FF]', screen: 'errors' as Screen },
  { id: 'seed',     icon: Moon,      title: 'Night Seed',   subtitle: '5 min · Pre-sleep',     time: '9:55 PM', color: 'text-primary',    bg: 'bg-primary/10',   screen: null as Screen | null },
];
...
const PranayamaSession: React.FC<{ onDone: () => void }> = ({ onDone }) => {
  ...
  if (finished) return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <CheckCircle2 size={48} className="text-[#15803D] mb-4" />
      ...
      <button onClick={onDone} className="px-8 py-4 bg-[#38BDF8] text-background rounded-2xl font-bold">Continue →</button>
    </div>
  );

  return (
    <div className="flex flex-col items-center justify-center py-8 text-center">
      <p className="text-[12px] uppercase tracking-[0.3em] text-[#78716C] font-bold mb-8">
      ...
      <div className="flex gap-2 mb-8">
        {BREATH_SEQUENCE.map((s, i) => (
          <div key={s.phase} className={`px-3 py-1 rounded-full text-[12px] font-bold uppercase transition-all ${
            isRunning && i === stepIdx ? 'bg-[#38BDF8] text-background' : 'bg-[#F0EEE9] text-[#78716C]'
          }`}>{s.label}</div>
        ))}
      </div>
        ...
        {(['schedule', 'pranayama', 'affirmations', 'rules'] as const).map(s => (
          <button key={s} onClick={() => setActiveSection(s)}
            className={`px-4 py-2 rounded-full text-[12px] font-bold uppercase tracking-widest whitespace-nowrap transition-all ${
              activeSection === s ? 'bg-primary text-background' : 'bg-[#F0EEE9] text-[#6B7280]'
            }`}>
            {s}
          </button>
        ))}
      ...
      <AnimatePresence mode="wait">
        {activeSection === 'schedule' && (
          <motion.div key="schedule" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <p className="text-[12px] uppercase tracking-widest text-[#78716C] font-bold mb-4 px-1">Today's Protocol</p>
            <div className="space-y-3">
              {DAILY_SCHEDULE.map((item) => (
                <div key={item.id} className={`card rounded-2xl p-4 flex items-center gap-4 border ${
                  completed.has(item.id) ? 'border-[#BBF7D0] opacity-60' : 'border-[#E8E5DF]'
                }`}>
            ...
            <div className="mt-6 p-4 rounded-2xl bg-[#F0EEE9] border border-[#E8E5DF] text-center">
              <p className="text-[12px] uppercase tracking-widest text-[#78716C] font-bold mb-1">Today's Progress</p>
              <p className="text-2xl font-ui font-bold">{completed.size}/{DAILY_SCHEDULE.length}</p>
              <div className="h-1 w-full bg-[#F0EEE9] rounded-full overflow-hidden mt-2">
          ...
          <motion.div key="affirmations" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div className="space-y-4">
              <div className="card rounded-3xl p-8 border border-primary/10 text-center">
                ...
                <p className="text-[12px] text-[#A8A29E]">Repeat 3 times before your session begins</p>
              </div>
              <div className="card rounded-2xl p-5 border border-[#E8E5DF]">
                <p className="text-[12px] uppercase tracking-widest text-[#78716C] font-bold mb-3">Ahamkara Programming</p>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={a.time} className="flex gap-3">
                      <span className="text-[12px] text-[#A8A29E] font-bold uppercase tracking-normal w-16 shrink-0 pt-0.5">{a.time}</span>
        ...
        {activeSection === 'rules' && (
          <motion.div key="rules" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            <div className="space-y-3">
              <p className="text-[12px] uppercase tracking-widest text-[#78716C] font-bold px-1 mb-4">30-Day Rules</p>
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                  className="card rounded-2xl p-4 border border-[#E8E5DF]">
